...
EXPERIMENT_LIST = [
    ...
    "quartiles",#compute quartiles of cosine similarities (by layer)#TODO remove
    "plot_fine",#create fine-grained plot
    "plot_coarse",#create coarse-grained plot (categories by layer)
    "plot_quartiles",#make boxplots of cosine similarities by layer#TODO rename
...
def analysis(args, model_name, cache_dir=None, checkpoint_value=None):
    ...
    if "randomness" in args.experiments and "randomness" not in data:
        ...
        data["randomness"] = utils.randomness_regions(model_data)#TODO this won't work when loading precomputed cosines
    if checkpoint_value is not None:

...
def neuron_choice(args, category_name, subset=None, baseline=True):
    """
    ...
    Args:
        ...
        category_index (int): neuron category from which to choose.
            0:"enrichment"
            1:"atypical enrichment"
            2:"conditional enrichment"
            3:"atypical conditional enrichment"
            4:"proportional change"
            5:"atypical proportional change"
            6:"orthogonal output"
            7:"depletion"
            8:"atypical depletion"
            9:"conditional depletion"
            10:"atypical conditional depletion"
        d_mlp (int):
    ...
    with open(f"{path}/data.pickle", 'rb') as f:
        data = pickle.load(f)
    neuron_tensor = torch.nonzero(data['categories']==category_index)
    ...
    if baseline:
        ...
        baseline_list = random_baseline(neuron_list, data['categories'], category_index)
        return neuron_list, baseline_list
    return neuron_list

def random_baseline(neuron_list, data_categories, category_index):
    ...
    Args:
        ...
        category_index (int): index of the current category
    ...
    for layer,number in enumerate(layer_counts):
        if number==0:
            continue
        baseline_sublist = list(torch.nonzero(data_categories[layer]!=category_index))
        if number<d_mlp/2:
            baseline_sublist = random.sample(baseline_sublist, number)
        else:
            print(
                f"Warning: category {CATEGORY_NAMES[category_index]} covers more than half of neurons in layer {layer} ({number} of {d_mlp})"
            )
